...
// visually identical to Iconsax Linear). Users may override any icon
// by dropping their own `<name>.svg` into the extension's `icons/ui/`
// folder — see README and tryFetchOverride() below.
//
...
// ---------- User-supplied SVG override mechanism ----------
//
// Drop a file at `icons/ui/<name>.svg` inside the extension folder to
// override the built-in icon for `<name>`. The file should be a complete
// `<svg>` element using `currentColor` for stroke/fill so it inherits
// the surrounding text color. Cached per-session to avoid repeated
// fetches; missing-file results are also cached as `null`.

const USER_OVERRIDE_DIR = 'icons/ui/';
const overrideCache = new Map();

function getOverrideUrl(name) {
  try {
    // Sanitize: only allow simple icon names (alpha, digit, dash, underscore).
    if (!/^[a-z0-9_-]+$/i.test(name)) return null;
    return browserAPI?.runtime?.getURL?.(`${USER_OVERRIDE_DIR}${name}.svg`) || null;
  } catch {
    return null;
  }
}
...
async function tryFetchOverride(name) {
  ...
  try {
    ...
    // Basic sanity check — must look like an SVG element.
    if (!text.startsWith('<svg')) {
      overrideCache.set(name, null);
      return null;
    }
    overrideCache.set(name, text);
    return text;
  } catch {
    overrideCache.set(name, null);
    return null;
...
// Synchronously hydrate placeholders from the built-in PATHS, then
// asynchronously attempt to swap in any user-supplied override SVGs.
// The sync path keeps first-paint instant; the async swap is invisible
...
export function applyIcons(root = document) {
  ...
  nodes.forEach((el) => {
    ...
    // Best-effort override swap. Fire-and-forget; errors are swallowed.
    tryFetchOverride(name).then((svg) => {
      if (svg && el.isConnected) {
        el.innerHTML = svg;
        el.dataset.iconOverride = '1';
      }
    }).catch(() => {});
  });
}
