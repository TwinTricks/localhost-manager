...
export async function pingPort(port, options = {}) {
  ...
  // 127.0.0.1 instead of localhost: in some Chrome SW contexts, "localhost"
  // resolves to IPv6 ::1 first and hangs. 127.0.0.1 skips DNS entirely.
  // Confirmed working with bare fetch in the user's SW console.
  return pingHostOnce(port, '127.0.0.1', 'http', timeoutMs);
}
